...
const api = {
	...
	// Windows WASAPI EXCLUDE-tree echo fix (#46): the main-world feeder ends ITS capture by id; main
	// ignores ids that are no longer current, so a late stop can't kill a newer share.
	stopWasapiLoopback: (captureId?: number) => invoke("wasapiLoopback:stopWasapiLoopback", captureId),
