<!-- ARCHIVED 2026-06-06 at v1.1 milestone close. Snapshot of REQUIREMENTS.md as shipped.
     All 12 requirements (v1.0: 7, v1.1: 5) complete. ECHO-02/03/04 verified viewer-side on
     Windows CI runs 27053219078 + 27054703843 (build 19045) — see 05-VERIFICATION.md.
     For the live project state see .planning/ROADMAP.md and .planning/PROJECT.md. -->

# Requirements: GoofCord — Windows Streaming Fixes  (ARCHIVED — v1.1)
...
- [x] **ECHO-02**: The echo fix works on current Windows — Windows 10 version 2004 (build 19041) and later, and Windows 11 — using the in-OS WASAPI per-process-tree EXCLUDE loopback (confirmed functional on the maintainer's build 19045 by official Discord's echo-free behaviour there; the documented "20348" minimum is over-stated — see `02-FINDINGS.md §2.3 UPDATE`)
- [x] **ECHO-03**: Existing screenshare/audio behaviour is preserved with no regression on Linux (patchcord), macOS, and on any Windows build where the per-process API is unavailable (graceful fallback to today's `"loopback"` behaviour — no crash, no worse than current)
- [x] **ECHO-04**: The native capability is implemented clean-room from the public Microsoft ApplicationLoopback sample (MIT-licensed; copyright notice retained); no Discord code or proprietary symbol layout is used
...
| ECHO-02 | Phase 4 | Complete |
| ECHO-03 | Phase 4 | Complete |
| ECHO-04 | Phase 4 | Complete |
...
- v1.1 requirements: 5 total — ECHO-01..04 → Phase 4, UPST-02 → Phase 5; all complete ✓ (Phase 3 is a de-risk gate owning no requirement)
- v2 / deferred (not in scope): WSTRM-01

---
*Requirements defined: 2026-05-29*
*Last updated: 2026-06-06 — v1.1 milestone close: ECHO-02/03/04 marked Complete (verified viewer-side on Windows CI runs 27053219078 + 27054703843, build 19045; see `05-VERIFICATION.md`). All 12 v1.0+v1.1 requirements complete.*
