...
### Fixes
- [ ] **KEY-01**: Non-alphanumeric global keybinds (OEM/punctuation keys: `]` `[` `;` `'` `,` `.` `/` `=` `-` `` ` `` `\`) register and fire on Windows. *Root cause (INV-02): `String.fromCharCode(domKeyCode)` at `src/windows/main/preload/keybinds.ts:53` yields Latin-1 garbage for OEM keyCodes; venbind matches the literal char. Fix = DOM-keyCode→char map. Pure-TS, no native work.*
- [ ] **STREAM-05**: The Windows "don't background an occluded window" Chromium switch is actually applied. *Root cause (INV-04 byproduct): `src/main.ts:67` sets `disable-disable-backgrounding-occluded-windows` (doubled `disable-`) — an unknown switch Chromium silently ignores, so occluded GoofCord windows can still be throttled/backgrounded, risking stream stalls when occluded. Fix = correct the flag name to `disable-backgrounding-occluded-windows`.*
