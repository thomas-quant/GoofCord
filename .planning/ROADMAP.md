...
- [x] **Phase 4: Native Clean-Room Exclude-Tree Addon + Integration** - Build the clean-room WASAPI process-tree EXCLUDE loopback `.node` addon and wire it into the Windows audio branch via the proven delivery path, with graceful fallback and no Linux/macOS regression (completed 2026-06-06)
...
- [x] 05-01-PLAN.md — Capture the rich (instrumented) two-device verification + the missing Windows build-number line, before any strip (D-14 step 1)
...
- [x] 05-05-PLAN.md — USER pushes the addon repo; build + packaging-assert the stripped/dependency-packaged Windows artifact (D-14 step 3 setup)
